...
parser.add_argument("--image", type=str, default=r'images/21_training.tif',
                    help="path to the image that used to train the model")
parser.add_argument("--mask_path", type=str, default=r'mask/21_mask.gif',
                    help="path to the mask file for training")
parser.add_argument("--epoch", type=int, default=1,
                    help="training epochs")
parser.add_argument("--checkpoint", type=str, default=r'pretrained/sam_vit_l_0b3195.pth',
                    help="path to the checkpoint of sam")
parser.add_argument("--model_name", default="vit_l", type=str,
...
def main(args):
    ...
    # 加载模型
    pth_path = '/root/sam/LearnablePromptSAM-main/weights/sam_vit_l_prompt_21_l2000_re.pth'
    checkpoint = torch.load(pth_path,map_location=torch.device('cpu'))
    model.load_state_dict(checkpoint)

    # pytorch转onnx
    onnx_path = '/root/sam/LearnablePromptSAM-main/pretrained/sam_21_l2000_re.onnx'
    input_shapes = [(1, 3, 1024, 1024), ]  # 模型输入tensor尺寸，例如NCHW，支持多个输入
    input_names = ['nchw_bgr', ]  # 模型输入tensor的名字，顺序与input_shapes对应。建议起容易理解的名字，例如nchw_rgb, nchw_bgr
    output_names = ['pred']  # 模型输出的名字，建议起容易理解的名字
    dynamic_batch_size = False  # 如果True，input_shapes中batch_size不起作用，方便同一个onnx转成不同max_batch_size的tensorrt engine
    net_to_onnx(model, onnx_path, input_shapes, input_names, output_names, dynamic_batch_size)
...
if __name__ == "__main__":
    ...
    # pred = (pred * 255).to(torch.uint8) # 将值缩放到-255范围，并转换为整数类型
    # image = Image.fromarray(pred.numpy())
    #
